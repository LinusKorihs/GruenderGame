...
public sealed class TutorialTrainingDummy : MonoBehaviour
{
    ...
    [Header("Feedback")]
    [SerializeField] private Vector3 feedbackLocalPosition = new Vector3(1.4f, 1.5f, 0f);
    [SerializeField] private Vector2 feedbackSize = new Vector2(2.8f, 0.8f);
    ...
    private void RefreshFeedback(float slowFraction)
    {
        ...
        feedbackText.text =
            $"Last Hit  {lastDamage:0.#}\n" +
            $"Recent Damage  {recentDamage:0.#}\n" +
            $"Slow  {slowFraction * 100f:0}%";
    }
    ...
    private void BuildFeedback()
    {
        ...
        backgroundObject.GetComponent<Image>().color = new Color(0.02f, 0.04f, 0.06f, 0.88f);
    ...
    private bool IsOccluded(Camera cameraToCheck)
    {
        ...
        RaycastHit[] hits = Physics.RaycastAll(origin, direction / distance, distance, Physics.DefaultRaycastLayers, QueryTriggerInteraction.Ignore);
